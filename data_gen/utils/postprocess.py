...
def get_mean_map(
        file_list=["data/ntu/aligned_data/pose/ntu_CS-pose_aligned.npz", "data/ntu/aligned_data/pose/ntu_CV-pose_aligned.npz"],
        batch_size=64
):
    for file in file_list:
        # First check if the _aligned.npz file exists - it must!
        if not osp.exists(file):
            print(f"File: \t{file}\nDoes not exist. Regenerate the aligned dataset and retry.")
            continue

        # Then check if the _mean.npz file exists - it mustn't!
        if osp.exists(file.replace("_aligned", "_aligned_mean")):
            print(f"Mean file {file.replace('_aligned', '_aligned_mean')} already exists!")
            continue
        ...
        start_time = time.time()
        with mmnpz.NpzWriter(file.replace("_aligned", "_aligned_mean")) as f:
        ...
        print(f"\t\tSaved data to: {file.replace('_aligned', '_aligned_mean')}", flush=True)
