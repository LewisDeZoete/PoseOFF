...
1. Checks whether each modality has completed the extraction of each class.
2. Check if samples need to be removed (i.e. They are full of zeros, containing no data...)
    These zero-samples are removed from the annotations file
"""
...
config_name = './config/infogcn2/ucf101/base.yaml'
...
print(f"Total original samples (in annotations): {len(list(annotations.keys()))}", flush=True)
...
print('Extract test complete.\nIncomplete classes:', flush=True)
for modality, num_incomplete_classes in incomplete.items():
    print(f'{modality}: {num_incomplete_classes}', flush=True)
    print(f'\t{len(num_incomplete_classes)} classes are incomplete', flush=True)
...
print(f"Total non-zero samples: {len(list(annotations.keys()))}", flush=True)
print(f"\t{zero_samples} removed", flush=True)
