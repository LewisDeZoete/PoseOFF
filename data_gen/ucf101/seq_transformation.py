...
def combine_data(all_paths):
    ...
    for idx, video_path in enumerate(all_paths):
        data = np.load(video_path) # C T V M
        no_frames = data.shape[1]
        if no_frames > 300:
            print(video_path)
            quit()
        data = rearrange(data, 'C T V M -> T (M V C)', C=C, T=no_frames, V=V, M=M)
        aligned_joints[idx, :no_frames] = data
...
def split_dataset(joints, labels, evaluation, dilation, flow_type, save_path, data_labels_root):
    """
    ...
    Args:
        ...
        dilation (int): Dilation factor used to extract the PoseOFF windows of optical flow.
        flow_type (str): Type of motion estimation used during PoseOFF extraction.
        save_path (str): The directory path where the resulting dataset file will be saved.
        data_labels_root (str): Root path to the labels for the UCF-101 dataset.
    ...
    save_name = osp.join(save_path, f'ucf101_0{evaluation}-poseoff_{flow_type}_D{dilation}.npz')
...
if __name__ == '__main__':
    ...
    for evaluation in range(1, 4):
        save_name = split_dataset(
            joints,
            labels=np.array(list(arg.feeder_args['labels'].values())),
            evaluation=evaluation,
            dilation=dilation,
            flow_type=flow_type,
            save_path=save_path, # ./data/ucf101/aligned_data/{flow_type}
