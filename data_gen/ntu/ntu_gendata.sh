...
export dataset=ntu # ntu, ntu120
export dilation=1
export flow_type=NF # LK, NF, RAFT, or WAFT
export mag_threshold=100 # For thresholding LK flow. Default is 100.


# Ensure the right flow type is being input...
_flow_types=("LK" "NF" "RAFT" "WAFT")
if [[ ${_flow_types[@]} =~ $flow_type ]]
then
    echo "Dataset: ${dataset}"
    echo "Dilation: ${dilation}"
    echo "Flow type: ${flow_type}"
    echo "Magnitude threshold: ${mag_threshold}"
else
    echo "Please edit the bash file, setting 'flow_type' to one of:"
    echo "'LK', 'NF', 'RAFT', or 'WAFT'"
    exit 1
fi
...
case $flow_type in
    ...
    # -------------------------------------------------------------
    # PoseOFF SAMPLING NORMAL FLOW (NF))
    # -------------------------------------------------------------
    "NF")
        # Both datasets have ~57000 samples so it would need >28 batches of 2000 to process
        flow_extract_id=$(sbatch --array=1-29 \
...
# # NOTE: Run only the following code if the above sections have failed...
# align_id=$(sbatch \
#     --export=ALL \
#     --job-name=${dataset}_seq_transform_D${dilation} \
#     --output=./logs/EXTRACT/${dataset}/${dataset}_seq_transform.out \
...
# ------------------------------------------
# GET MEAN MAP - TODO: Fix get_mean_map.py
# This *should* be handled by seq_transformation.sh...
# ------------------------------------------
# sbatch --export=ALL \
