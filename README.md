...
- Processes in batches of 2000 by default, gets *flow* data surrounding each keypoint saving only the flow windows as temporary files for each batch as pickle files which are concatenated with `concat_flow`.
