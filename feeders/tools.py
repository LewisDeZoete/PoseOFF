...
def valid_crop_resize(
    ...
    try:
        ...
        logging.debug(f"\tvalid size: {valid_size}")
        return None
    except Exception as e:
        logging.debug(f"ALTERNATE EXCEPTION: {e}")
        return None
    data = (
        data.contiguous()
...
def skel_displacement(data_numpy):
    '''Calculate the displacement of skeleton joints between frames.'''
    # Remove the existing flow channels if there are any...
    data_numpy = data_numpy[:3, ...]
    C, T, V, M = data_numpy.shape

    # Compute frame-to-frame velocity: shape (3, T-1, V, M)
    velocity = data_numpy[:, 1:, :, :] - data_numpy[:, :-1, :, :]

    # Trim spatial data to T-1 to match velocity length
    spatial = data_numpy[:, :-1, :, :] # (3, T-1, V, M)

    data_numpy = np.concatenate([spatial, velocity], axis=0)
