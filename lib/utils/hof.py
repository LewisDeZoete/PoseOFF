...
def hog2image(hogArray, imageSize=[32,32],orientations=9,pixels_per_cell=(8, 8),cells_per_block=(3, 3)):
    from numpy import pi, arctan2, cos, sin
...
def calcHog(img, stack:bool=True, resize_val=1):
    fd, hog_image = hog(
        img,
        orientations=9,
    ...
    hog_image_rescaled = exposure.adjust_gamma(hog_image, 10)
    hog_image_resized = cv2.resize(hog_image_rescaled, (hog_image_rescaled.shape[1]*resize_val, hog_image_rescaled.shape[0]*resize_val))
    if stack:
        hog_image = cv2.merge([hog_image_resized, hog_image_resized, hog_image_resized]).astype('uint8')
        return hog_image
    else:
        return hog_image_resized



def hof(flow, orientations=9, pixels_per_cell=(8, 8),
        cells_per_block=(3, 3), visualise=False, normalise=False, 
        resize_val=1, motion_threshold=1.):
    ...
    resize_val : proportion value for resizing, visualise
        must be true.
    ...
    * Dalal, N and Triggs, B, Histograms of Oriented Gradients for
    ...
    If I could outsource this to c++ that would be great...
    ...
    if visualise:
        hof_image_rescaled = exposure.adjust_gamma(hof_image, 20)
        hof_image_resized = cv2.resize(hof_image_rescaled, (hof_image_rescaled.shape[1]*resize_val, hof_image_rescaled.shape[0]*resize_val))
        hof_image = cv2.merge([hof_image_resized, hof_image_resized, hof_image_resized]).astype('uint8')
