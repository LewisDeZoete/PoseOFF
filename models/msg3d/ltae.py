...
code: https://github.com/VSainteuf/lightweight-temporal-attention-pytorch/blob/master/models/ltae.py
...
if __name__ == '__main__':
    ...
    len_seq_max = 300
    out_features = 16
    ltae = LTAE(in_channels=2*window_size**2, 
                n_head=8, 
                d_k=4,
                n_neurons=[128,out_features],
                d_model=128,
                len_max_seq=len_seq_max,
                return_att=True)
    pytorch_total_params = sum(p.numel() for p in ltae.parameters())
    print(f'Number model parameters: {pytorch_total_params:0.2e}')

    N, C, T, V, M = 256, 2*window_size**2, len_seq_max, 17, 2
    input = torch.randn((N,C,T,V,M))
    input = input.permute(0,3,4,2,1).contiguous().view(N*V*M, T, C)
