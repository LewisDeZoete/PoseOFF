...
from model_utils import bn_init, conv_init
...
class unit_gcn(nn.Module):
    ...
    def __init__(self,
        ...
        # TODO: For all of these modules, take away the mmcv style of creating layers...
        self.norm_cfg = norm if isinstance(norm, dict) else dict(type=norm)
...
if __name__ == '__main__':
    from models.stgcn2.stgcn2 import STGCNBlock
    from config.argclass import ArgClass

    arg = ArgClass(f"config/stgcn2/ntu/TEST.yaml")
    graph = arg.import_class(arg.model_args['graph'])()
    A = torch.tensor(graph.A, dtype=torch.float32, requires_grad=False)

    in_channels = 53
    base_channels = 64

    gcn = STGCNBlock(in_channels, base_channels, A.clone(), 1, residual=False)

    N, C, T, V, M = (8, 53, 64, 25, 2)
    x = torch.randn((N*M, C, T, V))
